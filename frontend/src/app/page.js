...
export default function Home() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      } else if (searchMode === 'realtime') {
        res = await searchRealtime({ query, filters: activeFilters, top_k: 20 });
        ...
        <form onSubmit={handleSearch} style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="flex gap-2">
              ...
              <button type="button" className={`btn-glass ${searchMode === 'realtime' ? 'active' : ''}`} style={{ background: searchMode === 'realtime' ? 'rgba(239, 68, 68, 0.2)' : '', color: searchMode === 'realtime' ? '#f87171' : '', borderColor: searchMode === 'realtime' ? '#ef4444' : '' }} onClick={() => setSearchMode('realtime')}>🔴 Realtime</button>
