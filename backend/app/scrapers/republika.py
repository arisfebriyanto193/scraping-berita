...
class RepublikaScraper(BaseScraper):
    ...
    def scrape_latest(self, max_articles: int = 20) -> List[ArticleData]:
        ...
        for section_url in self.SECTIONS:
            if len(articles) >= max_articles:
                break
            self.logger.info(f"Scraping Republika: {section_url}")
            ...
            for url in links:
                ...
                if article and article.is_valid():
                    ...
                    self.logger.info(f"✅ Republika: {article.title[:60]}")

        self.logger.info(f"Total scraped dari Republika: {len(articles)} artikel")
        return articles
    ...
    def _extract_article_links(self, soup: BeautifulSoup) -> List[str]:
        ...
        for tag in soup.find_all("a", href=True):
            ...
            if not href.startswith("http"):
                href = f"https://www.republika.co.id{href}" if href.startswith("/") else href
            if "republika.co.id" in href and "/berita/" in href:
    ...
    def parse_article(self, url: str) -> Optional[ArticleData]:
        ...
        try:
            # Title
            title = ""
            title_el = soup.find("h1", class_="article-title") or soup.find("h1")
            if title_el:
            ...
            content = ""
            content_el = (
                soup.find("div", class_="article-detail-content")
                or soup.find("div", {"id": "article-detail-content"})
            ...
            # Date
            published_date = None
            date_el = soup.find("time") or soup.find("span", class_="date-post")
            if date_el:
            ...
            # Author
            author = None
            author_el = soup.find("span", class_="reporter-author") or soup.find("a", rel="author")
            if author_el:
            ...
            self.logger.error(f"Error parse Republika {url}: {e}")
            return None
