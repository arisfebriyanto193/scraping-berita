...
class Liputan6Scraper(BaseScraper):
    ...
    def scrape_latest(self, max_articles: int = 20) -> List[ArticleData]:
        ...
        for section_url in self.SECTIONS:
            if len(articles) >= max_articles:
                break
            self.logger.info(f"Scraping Liputan6: {section_url}")
            ...
            for url in links:
                ...
                if article and article.is_valid():
                    ...
                    self.logger.info(f"✅ Liputan6: {article.title[:60]}")

        self.logger.info(f"Total scraped dari Liputan6: {len(articles)} artikel")
        return articles
    ...
    def parse_article(self, url: str) -> Optional[ArticleData]:
        ...
        try:
            ...
            content = ""
            content_el = (
                soup.find("div", class_="article-content-body")
            ...
            date_el = (
                ...
                or soup.find("time")
                or soup.find("span", class_="article-date-published")
            )
            if date_el:
            ...
            self.logger.error(f"Error parse Liputan6 {url}: {e}")
            return None
