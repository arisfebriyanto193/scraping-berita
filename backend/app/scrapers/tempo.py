...
class TempoScraper(BaseScraper):
    ...
    def scrape_latest(self, max_articles: int = 20) -> List[ArticleData]:
        ...
        for section_url in self.SECTIONS:
            if len(articles) >= max_articles:
                break
            self.logger.info(f"Scraping Tempo: {section_url}")
            ...
            for url in links:
                ...
                if article and article.is_valid():
                    ...
                    self.logger.info(f"✅ Tempo: {article.title[:60]}")

        self.logger.info(f"Total scraped dari Tempo: {len(articles)} artikel")
        return articles
    ...
    def parse_article(self, url: str) -> Optional[ArticleData]:
        ...
        try:
            ...
            # Content - Tempo pakai div#article-detail-content
            content = ""
            content_el = (
                soup.find("div", {"id": "article-detail-content"})
                or soup.find("div", class_="detail-in")
            ...
            # Date
            published_date = None
            date_el = soup.find("time") or soup.find("p", class_="date")
            if date_el:
            ...
            # Author
            author = None
            author_el = soup.find("p", class_="reporter") or soup.find("a", rel="author")
            if author_el:
            ...
            self.logger.error(f"Error parse Tempo {url}: {e}")
            return None
    ...
    def _parse_date(self, date_text: str) -> Optional[datetime]:
        ...
        try:
            ...
            # Tempo format: "Senin, 15 Januari 2024 10:00 WIB"
